...
def pawnmove(n, x0, y0, x1, y1):
    if x1 < 8 and y1 < 8 and y1 >= 0 and x1 >= 0:
        if n > 0:
            if ((y1 == y0 and (x1-x0 == 1 or (x1-x0 == 2 and x0 == 1)) and board[x1][y1] == 0) or 
                (x1-x0 == 1 and board[x1][y1] < 0 and (y1 - y0 == 1 or y1 - y0 == -1))):
                return True
            return False
        elif n < 0:
            if ((y1 == y0 and (x0-x1 == 1 or (x0-x1 == 2 and x0 == 6)) and board[x1][y1] == 0) or
...
def movepawn():
    ...
    if (move > 0 and turn == 0) or (move < 0 and turn == 1):
        for i in range(8):
            if move in board[i]:
                if pawnmove(move, i, board[i].index(move), movetox, movetoy):
